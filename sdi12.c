...
#define MAX_RESPONSE_TIMEOUT (10000) // Millisecnods
...
#define SDI12_BREAK_SPEC_US (12000)
#define SDI12_POST_BREAK_MARKING_US (9000)
#define SDI12_POST_BREAK_MARKING_SPEC_US (8333)
 ...
 *      - ESP_OK  configuration and installation OK
 */
static esp_err_t config_rmt_as_rx(p_sdi12_bus_t *p_bus, bool increaseIdleThreshold)
{
    ...
    if (increaseIdleThreshold)
        rmt_rx.rx_config.idle_threshold = SDI12_BREAK_SPEC_US + 500;
 ...
 */
static esp_err_t parse_response(p_sdi12_bus_t *p_bus, rmt_item32_t *items, size_t items_length, size_t item_index)
{
    ...
    while (item_index < items_length)
    {
        ...
        // when start_item_index is 1, it indicates there was a break/mark already received from wait_receive
        // check buffer to see if the ! character was received, so we can stop processing remaining buffer bits
        // we do this since the rest of the buffer can be a response from another device on the SDI-12 bus
        if (start_item_index == 1)
        {
            bool foundBang = false;
            for (size_t i = 0; i < p_bus->response_buffer_length; i++)
            {
                // if this char is null terminator and last char was !
                if (p_bus->response_buffer[i] == '\0' && p_bus->response_buffer[i - 1] == '!')
                {
                    foundBang = true;
                    break;
                }
...
static esp_err_t read_response_line(p_sdi12_bus_t *p_bus, uint32_t timeout)
{
    ...
    ret = config_rmt_as_rx(p_bus, false);
    ...
    aux_timeout = aux_timeout > MAX_RESPONSE_TIMEOUT ? MAX_RESPONSE_TIMEOUT : aux_timeout;
    ...
    if (ret == ESP_OK && rb)
    {
        do
        {
            ...
            if (items)
            {
                ...
                ret = parse_response(p_bus, items, length, 0);
            }
            else
            {
                ret = ESP_ERR_TIMEOUT;
...
static esp_err_t send_cmd(p_sdi12_bus_t *p_bus, const char *cmd, bool sendBreakMark)
{
    if (!cmd)
        return ESP_ERR_INVALID_ARG;
    ...
    uint8_t rmt_items_length = strlen(cmd) * 5;
    if (sendBreakMark)
        rmt_items_length = 1+ strlen(cmd) * 5;
...
{
    esp_err_t ret = send_cmd(p_bus, cmd, true);
...
static esp_err_t start_any_measurement(sdi12_bus_t *bus, char address, char measurement_type, uint8_t index, uint8_t *ready_seconds, uint8_t *measurements, uint32_t timeout)
{
    ...
    if (ret == ESP_OK)
    {
        ...
        if (ret == ESP_OK)
        {
            ...
            if (strlen(p_bus->response_buffer) < 5)
            {
                ESP_LOGE(TAG, "Invalid measure response: %s", p_bus->response_buffer);
                ret = ESP_FAIL;
                goto end;
            }
...
static esp_err_t wait_receive(sdi12_bus_t *bus, const char *out_buffer, size_t out_buffer_length)
{
    p_sdi12_bus_t *p_bus = __containerof(bus, p_sdi12_bus_t, parent);
    SDI12_CHECK(p_bus, "BUS is NULL", err);

    // START SEPARATE FUNCTION TO WAIT FOR RESPONSE
    esp_err_t ret = config_rmt_as_rx(p_bus, true);
    ...
    rmt_item32_t *items;
    size_t length = 0;
    uint64_t aux_timeout = 45000;
    RingbufHandle_t rb = NULL;
    // uint32_t loopbackTime = 0;
    ...
    if (ret == ESP_OK && rb)
    {
        do
        {
            ...
            if (items)
            {
                // Check if GPIO stays high for 12200us and low for 9000us (SDI-12 break and mark)
                if ((items[0].duration0 >= SDI12_BREAK_SPEC_US && items[0].level0 == 1) && (items[0].duration1 >= SDI12_POST_BREAK_MARKING_SPEC_US && items[0].level1 == 0))
                {
                    // ESP_LOGW(TAG, "Got break and mark!");
                    ...
                    parse_response(p_bus, items, length, 1);

                    // ESP_LOGI(TAG, "wait_receive.response_buffer: %s", p_bus->response_buffer);
                    snprintf((char *)out_buffer, out_buffer_length, p_bus->response_buffer);

                    //empty the response buffer
                    p_bus->response_buffer[0] = '\0';

                    //uninstall the rx driver to clear everything out
                    rmt_driver_uninstall(p_bus->rmt_channel);
                    p_bus->rmt_mode = 3;
                ...
                {
                    // ESP_LOGI(TAG, "No break and mark");
                }
            }
...
static esp_err_t send_response(sdi12_bus_t *bus, const char *response)
{
    //put the command together with SDI-12 required<CR><LF>
    char cmdToSend[strlen(response) + 3];
    snprintf(cmdToSend, sizeof(cmdToSend), "%s%c%c%c", response, '\r', '\n', '\0');
    // ESP_LOGW(TAG, "send_response: %s", cmdToSend);

    p_sdi12_bus_t *p_bus = __containerof(bus, p_sdi12_bus_t, parent);
    SDI12_CHECK(p_bus, "BUS is NULL", err);

    esp_err_t err = send_cmd(p_bus, cmdToSend, false);
...
sdi12_bus_t *sdi12_bus_init(sdi12_bus_config_t *config)
{
    ...
    p_sdi12_bus->parent.wait_receive = wait_receive;
    p_sdi12_bus->parent.send_response = send_response;
